...
def _make_target(cfg_t):
    ...
    # <target>_dq: internal staging — transformed rows + one boolean col per rule
    # (_rule_<safe>) + a _quarantine flag (true iff any FAIL rule is violated) + a
    # _failed_rules array (the FAIL rules each row broke). The clean target derives from
    # this; the app rolls quarantined rows (_quarantine=true) into the CENTRAL
    # mf_dq_quarantine table after the run. There is NO per-target quarantine table —
    # quarantine is centralized framework-wide.
    def _build_dq(_cfg=cfg_t, _rules=rules, _fail=fail_rules):
        ...
        df = df.withColumn("_quarantine", quarantine)
        # _failed_rules: the FAIL rules each row broke (only meaningful when _quarantine).
        present = [r for r in _fail if f"_rule_{_sanitize(r['name'])}" in df.columns]
        if present:
            marks = F.array(*[F.when(~F.col(f"_rule_{_sanitize(r['name'])}"), F.lit(r["name"])) for r in present])
            df = df.withColumn("_failed_rules", F.array_except(marks, F.array(F.lit(None))))
        else:
            df = df.withColumn("_failed_rules", F.array().cast("array<string>"))
        return df
    _build_dq.__name__ = f"build_{dq_name}"
    dlt.table(name=dq_name, comment=f"{name}: staging — rows + per-rule DQ booleans + _quarantine flag + _failed_rules")(_build_dq)

    # Clean target: rows NOT quarantined (FAIL violators kept OUT), marker cols dropped.
    def _build_target(_dq=dq_name):
        df = dlt.read(_dq).filter(~F.col("_quarantine"))
        drop = [c for c in df.columns if c.startswith("_rule_")] + ["_quarantine", "_failed_rules"]
        return df.drop(*drop)
